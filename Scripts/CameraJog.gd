...
func process_input(event: InputEvent) -> State:
	...
	if Input.is_action_just_released("ctrl"):
		...
		if parent.velocity.x != 0 and parent.velocity.z != 0 and GlobalVariables._jogging == false and Input.is_action_pressed("ctrl"): #if moving, stopping jogging, and in camera mode
			return _camera_walk_state
		if parent.velocity.x != 0 and parent.velocity.z != 0 and Input.is_action_pressed("run") and Input.is_action_pressed("ctrl"): #if moving, pressing run, and in camera mode
