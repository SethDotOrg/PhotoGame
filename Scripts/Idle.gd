...
func process_input(event: InputEvent) -> State:
	if parent.is_on_floor():
		...
		if (Input.is_action_pressed("move_left") or Input.is_action_pressed("move_right") or Input.is_action_pressed("move_forward") or Input.is_action_pressed("move_back")) and parent.is_on_wall() == false and parent.is_on_floor() == true: 
			return walk_state
