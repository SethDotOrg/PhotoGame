...
func _ready():
	...
	#formula to change the slider value to decibel
	...
	#set the sliders to show the volume that the audio bus they point to is set
...
func _on_master_volume_slider_drag_ended(value_changed):
	if(get_parent()._player_ui.MENU_STATES.SOUND_OPTIONS_STATE):
		if value_changed == true:
			...
			if (slider_value < 0.01):#set the audio bus to mute if the slider is at a really low value
...
func _on_sound_effect_volume_slider_drag_ended(value_changed):
	if(get_parent()._player_ui.MENU_STATES.SOUND_OPTIONS_STATE):
		if value_changed == true:
			...
			if (slider_value < 0.01):if (slider_value < 0.01):#set the audio bus to mute if the slider is at a really low value
	...
	if(get_parent()._player_ui.MENU_STATES.SOUND_OPTIONS_STATE):
		if value_changed == true:
			...
			if (slider_value < 0.01):
				AudioServer.set_bus_mute(2, true)#set audio bus to must if its a really low value
			elif (slider_value >= 0.01):
